...
//
// ⚠ WHAT A GREEN HERE DOES **NOT** LICENCE, MEASURED RATHER THAN ASSUMED. Drop
// `a.requireSession` from docsAskAI and the ENTIRE bff suite stays green — this test included.
// The refusal is produced three times over: by the wrapper, by docsWorkspaceFor →
// trackWorkspaceFor, and again by forwardProduct, each with the SAME 401 and byte-identical
// body, so no black-box assertion can say which one answered. The PROPERTY asserted below is
// real and worth pinning; what is not available from it is the inference "therefore this
// handler checks the session". On this route the wrapper is defence-in-depth.
//
// ⚠ AND THE COMPLETENESS GUARD CANNOT COVER FOR IT — its 405 branch EXCUSES exactly this
// defect. TestEveryMountedRoute_RefusesAnonymousRead drives GET, and dropping a write-only
// route's gate is precisely what turns that route's anonymous GET from a 401 into the 405 the
// test waves through as "no read surface". Measured one mutation at a time across all five
// write-only routes: /api/lxc/checkout, /api/lens/convert and /api/keys/{id} are each caught by
// a hand-written companion like this one — and /api/pooling is caught by NOTHING.
