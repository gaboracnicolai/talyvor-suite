...
// not rendering. Overview.tsx:342 is the SAME seam written correctly (`ledger.isError` →
...
describe('a 500 next to a 401 is still a 500', () => {
  ...
  it('/ (Overview) does this correctly — the control that says the product already knows the answer', async () => {
    ...
    at('/')
    await settled()
    // Overview.tsx:342 — `ledger.isError ? <Failed what="the mint ledger" error={ledger.error} />`.
