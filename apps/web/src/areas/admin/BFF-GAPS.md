...
2. **⭐ The authz decision — DECIDED (repo owner, 2026-07-22): admin gets its own allowlist.**
   A trial user is not an operator, and edge-infra state — topology, cert expiry, node status —
   is not theirs to see. The admin routes are gated by **`ADMIN_ALLOWED_EMAILS`**, checked after
   `requireSession`, **separate from `OIDC_ALLOWED_EMAILS`**, and **defaulting to EMPTY** so the
   admin surface is unreachable until someone is deliberately named. Build it exactly so; do not
   re-derive.
